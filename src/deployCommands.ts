...
export default async function DeployCommands() {
    ...
    try {
        ...
        await rest.put("DISCORD_GUILD_ID" in process.env ? Routes.applicationGuildCommands(process.env.DISCORD_CLIENT_ID!, process.env.DISCORD_GUILD_ID!) : Routes.applicationGuildCommands(process.env.DISCORD_CLIENT_ID!), {
            body: commands
        });
